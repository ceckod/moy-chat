...
const Settings = {
  ...
  async testKeys() {
    ...
    // Gemini — ползваме конкретно "gemini-3.6-flash" (текущ актуален модел към момента).
    // Забележка: Google сменя/спира модели изненадващо често — ако това пак гръмне с 404,
    // провери https://ai.google.dev/gemini-api/docs/models за най-новото име на модела.
    if (!k.gemini) lines.push("Gemini: ⚪ няма ключ");
    else {
      try {
        const r = await fetchTimeout(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent?key=${k.gemini}`, {
...
async function callGemini(prompt, useSearch = false) {
  ...
  const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent?key=${k.gemini}`;
