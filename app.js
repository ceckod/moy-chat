...
const Settings = {
  ...
  async testKeys() {
    ...
    // Gemini — ползваме "gemini-flash-latest" (auto-updating alias от Google), защото Google
    if (!k.gemini) lines.push("Gemini: ⚪ няма ключ");
    else {
      try {
        const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent?key=${k.gemini}`, {
...
async function callGemini(prompt, useSearch = false) {
  ...
  const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent?key=${k.gemini}`;
