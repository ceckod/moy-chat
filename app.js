...
const Settings = {
  ...
  async testKeys() {
    ...
    // Gemini — gemini-2.0-flash е спрян от Google (юни 2026); ползваме текущия gemini-2.5-flash
    if (!k.gemini) lines.push("Gemini: ⚪ няма ключ");
    else {
      try {
        const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${k.gemini}`, {
...
async function callGemini(prompt, useSearch = false) {
  ...
  const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${k.gemini}`;
...
Опиши стил, цветова палитра, композиция, настроение. Максимум 4-5 изречения, само промпта.`;
  ...
  async generateCoverImage() {
    ...
    try {
      ...
      const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image:generateContent?key=${k.gemini}`;
      const res = await fetch(proxied(url), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
