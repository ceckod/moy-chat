...
const Settings = {
  ...
  async testKeys() {
    ...
    // Gemini — ползваме "gemini-3.5-flash" (потвърден безплатен tier към юли 2026).
    // По-новият "gemini-3.6-flash" гърмеше с 429 (RESOURCE_EXHAUSTED) на безплатни ключове —
    // вероятно все още няма отворен free tier, само на дни от пускането си.
    // Ако това пак спре да работи, провери https://ai.google.dev/gemini-api/docs/models
    if (!k.gemini) lines.push("Gemini: ⚪ няма ключ");
    else {
      try {
        const r = await fetchTimeout(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key=${k.gemini}`, {
  ...
  // ВАЖНО: не прави реална Gemini заявка при ВСЯКО презареждане — това хаби ценна дневна квота
  // на безплатния tier само от отваряне/презареждане на страницата. Реална проверка се прави
  // максимум веднъж на 30 мин; между тях просто показва последния известен резултат.
  async silentHealthCheck() {
    ...
    const CACHE_KEY = "cdb_health_cache_v1";
    const THROTTLE_MS = 30 * 60 * 1000;
    let cached = null;
    try { cached = JSON.parse(localStorage.getItem(CACHE_KEY) || "null"); } catch (e) {}
    if (cached && (Date.now() - cached.time) < THROTTLE_MS) {
      if (txt) txt.textContent = cached.ok ? "Всички системи активни" : "Провери ключовете (кеширано)";
      if (dot) dot.style.background = cached.ok ? "var(--green)" : "var(--red)";
      return;
    }
    ...
    try {
      ...
      localStorage.setItem(CACHE_KEY, JSON.stringify({ ok: true, time: Date.now() }));
    } catch (e) {
      if (txt) txt.textContent = "Провери ключовете";
      if (dot) dot.style.background = "var(--red)";
      toast("⚠️ " + e.message + " — виж Настройки → API Ключове");
      localStorage.setItem(CACHE_KEY, JSON.stringify({ ok: false, time: Date.now() }));
    }
  },
...
async function callClaude(prompt, maxTokens = 1200) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 429) throw new Error("Claude: изчерпан лимит за момента (429) — изчакай малко и опитай пак.");
...
async function callGemini(prompt, useSearch = false) {
  ...
  const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key=${k.gemini}`;
  ...
  if (!res.ok) {
    const t = await res.text();
    if (res.status === 429) throw new Error("Gemini: изчерпан безплатен лимит за момента (429) — изчакай няколко минути (RPM) или до утре (дневен лимит) и опитай пак.");
