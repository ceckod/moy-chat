...
const SystemTest = {
  ...
  // Очакван формат от prompt-а: всеки ред от нов номериран елемент
  // започва с "N. **Заглавие**" — стандартен маркдаун стил, който повечето
  // модели следват естествено, а тук допълнително им го изискваме изрично
  // в prompt-а.
  //
  // ВАЖНО: маркерът е СТРОГО номериран ("1.", "2)" — не bullet точки
  // ("-•*"). Причината: "мислещи"/reasoning модели (особено някои
  // безплатни OpenRouter модели) понякога изливат цялото си разсъждение
  // ПРЕДИ финалния отговор — чернови бележки от типа "* **Selection 1:
  // ...**" (bullet маркер, не номер). Ако приемахме и bullet маркери,
  // тези чернови редове се хващаха като отделни "идеи" в архива, вместо
  // само чистия финален номериран списък.
  _splitIdeas(text) {
    const markerRe = /^\s*\d+[.)]\s*\*\*(.+?)\*\*/gm;
    let marks = [...text.matchAll(markerRe)];
    if (!marks.length) {
    ...
    // Допълнителна защита: ако номерацията "рестартира" от 1 нанякъде
    // по-нататък в текста (напр. модел номерира и черновата си "1,2,3...",
    // после истинският финален отговор пак започва от "1,2,3..."), пазим
    // САМО последната такава поредица — тя е реалният финален отговор.
    let lastRestart = 0;
    for (let i = 1; i < marks.length; i++) {
      const num = parseInt(marks[i][0], 10);
      const prevNum = parseInt(marks[i - 1][0], 10);
      if (num === 1 && prevNum !== 1) lastRestart = i;
    }
    marks = marks.slice(lastRestart);
...
ФОРМАТ (ЗАДЪЛЖИТЕЛЕН): всяка идея на СВОЙ номериран ред, който започва с КРАТКО ЗАГЛАВИЕ в **удебелен** текст, после тире и описание — напр. "1. **Hook Strength Score** – кратко описание...". Заглавието е важно — по него потребителят ще разпознава коя идея е изградил, за да не му я предлагаш пак. НЕ показвай разсъждения, чернови варианти или стъпки на мислене — директно само финалния номериран списък. Кратко, на български, без излишен увод.`;
