...
**Версия:** 1.11.0
**Последна промяна:** 2026-08-08, 08:10 (Europe/Sofia)
...
- **"🔒 Достъп до dashboard-а" (Auth Gate) е екран за поверителност, НЕ
  ...
  ключалката). Ако забравиш логина си: отвори DevTools конзолата (F12) на
  страницата и изпълни `localStorage.removeItem('cdb_auth_gate_hash_v1')`,
  после презареди — данните ти не се трият, само самата ключалка. Няма
  бутон за това в интерфейса нарочно — иначе би бил байпас за всеки друг,
  не само теб.
