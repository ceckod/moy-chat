...
**Версия:** 1.30.1
**Последна промяна:** 2026-08-16 (Europe/Sofia) — 🐛 Bugfix: refresh вече не връща на началния екран + Service Worker update fix (без нужда от затваряне на таба) — виж Changelog
...
### v1.30.1 — 2026-08-16 (bugfix — навигация + Service Worker update)
- 🐛 **`js/nav.js`** — `Nav.init()` игнорираше `location.hash` при зареждане и винаги рестартираше на `"dashboard"`, независимо на кой view е бил потребителят при refresh. Поправено: чете hash-а и остава на текущия view, ако е валиден.
- 🐛 **`index.html`** — регистрацията на Service Worker вече ползва `{ updateViaCache: "none" }`, за да не позволява на браузъра да сервира стар кеширан `sw.js` вместо да провери мрежата — преди това понякога изискваше затваряне и повторно отваряне на таба, за да се хванат нови промени.
- ⚠️ **Повторно потвърдено:** 31-те мъртви/дублирани файла, документирани като изтрити в v1.29.0 и отново в `PROJECT_STATE.md` (2026-08-15), пак присъстваха в качения ZIP — виж записа в `PROJECT_STATE.md` за детайли и препоръка занапред.
- ✅ `npm test` (85/85) минава преди и след двете промени.

### v1.30.0 — 2026-08-13 (Suno Audio Preview Player)
- 🆕 **`js/suno-preview.js`** (`SunoPreview`) + нов екран в "Инструменти" (`#view-suno-preview`) — вграден HTML5 `<audio controls>` плейър за преглед на Suno AI output директно в браузъра, без сваляне. Два режима (табове):
  - 🔗 **Линк** — paste на директен Suno CDN audio URL → нула download, чист streaming preview.
  - 📁 **Локален файл** — file picker → инстантен preview през `URL.createObjectURL()`, за вече свалени файлове.
  - История на последните 20 прегледа (localStorage) — Suno линковете могат да се пуснат отново с 1 клик; локалните файлове пазят само име/дата (браузърно ограничение — Object URL не надживява презареждане).
  - Изолиран модул — не изисква VLC или друга библиотека, не е обвързан с текущия проект/песен (виж `MODULE-MAP.md` Раздел 9b).
- ✅ `npm test` (73/73) непроменен — новият модул не пипа съществуваща логика.
