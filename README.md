...
**Версия:** 1.10.1
**Последна промяна:** 2026-08-08, 07:40 (Europe/Sofia)
...
- **"🔒 Достъп до dashboard-а" (Auth Gate) е екран за поверителност, НЕ
  ...
  без сървър, който да я налага. Спира случаен поглед, не спира някой с
  достъп до DevTools (може директно да изтрие ключалката). Ако забравиш
  паролата си: отвори DevTools конзолата (F12) на страницата и изпълни
  `localStorage.removeItem('cdb_auth_gate_hash_v1')`, после презареди —
  данните ти не се трият, само самата ключалка. Няма бутон за това в
  интерфейса нарочно — иначе би бил байпас за всеки друг, не само теб.
