...
## 2026-08-16 — Нов одит (нова Claude.ai сесия, качен `FULL.zip`)

**Контекст:** потребителят качи цялото repo в нов разговор, без preamble — сесията не е продължение на 2026-08-15. Поискан е пълен одит на сайта (без `visualizer.html`).

**Открити и поправени 2 реални бъга в живия код:**
1. `js/nav.js` → `Nav.init()` игнорираше `location.hash`, винаги рестартираше на `"dashboard"` при refresh. **Поправено** — чете hash-а, остава на текущия view, ако е валиден.
2. `index.html` → SW регистрацията нямаше `updateViaCache: "none"`, което забавяше засичането на нова версия на `sw.js` и изискваше затваряне/отваряне на таба. **Поправено.**

И двете потвърдени с `npm test` (85/85) преди/след.

**⚠️ Процесен проблем, потвърден трети път:** 31-те мъртви/дублирани файла (root JS дубликати + orphan HTML + 3 дребни дубликата), документирани като изтрити в README v1.29.0 (2026-08-13) и отново реално изтрити в сесия от 2026-08-15 (виж `PROJECT_STATE.md`), **пак присъстваха** в качения `FULL.zip`. Първият опит за поправка на nav бъга в тази сесия дори случайно се приложи върху мъртвото root копие на `nav.js` (без ефект), точно заради тези дубликати — потвърждава риска, документиран още в `scripts/cleanup_dead_files.py`.

Root cause хипотеза (не потвърдена от потребителя): или (а) потребителят archive-ва/експортва по-стар snapshot на repo-то вместо последния commit, или (б) ZIP-овете с fix-ове от предходни сесии не се качват реално обратно през `incoming/` в GitHub, така че живият repo никога не вижда промените. И двете обясняват защо променен код "изчезва" между сесии.

**Препоръка занапред:** преди следваща сесия — потвърди директно в GitHub (github.com/ceckod/moy-chat), че commit-ът от 2026-08-15 (`cleanup_dead_files.py --apply`, 31/31 изтрити) реално стои в `main` branch-а. Ако не стои — това е доказателство за хипотеза (б), не (а).

**Нова находка (сигурност):** `ai-model-finder/worker.js` (Cloudflare Worker) е отворен прокси — приема произволен `?url=` параметър от всеки в интернет, връща резултата с `Access-Control-Allow-Origin: '*'`, без allow-list или auth проверка. SSRF/злоупотреба риск (чужд трафик през твоята Cloudflare сметка). Не е поправено в тази сесия — само флагнато, чака решение.

**Дребни пропуски, флагнати (не поправени):**
- `sw.js` → `SHELL_FILES` липсват 8 реално ползвани файла (напр. `js/system-update.js`, `js/suno-preview.js`) — не са фатални, само не се pre-cache-ват за offline при първо зареждане.
- `manifest.json` → `"icons": []` — PWA "Add to Home Screen" ще ползва generic икона.

**Статус: 2 бъга поправени и потвърдени. Мъртвите файлове НЕ са изтрити тази сесия** (потребителят предпочете първо пълен одит) — `scripts/cleanup_dead_files.py --apply` чака изрично разрешение. Worker.js сигурността чака решение.
