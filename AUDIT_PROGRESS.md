...
## [2026-08-11, продължение] Фикс след първото реално пускане — портфолио-относителна нормализация + Discogs Community

**Контекст:** първото живо пускане на `niche-scores.yml` мина успешно
(30 ниши, discovery работи — "Dai Dai" и "Shiesty Season" открити
автоматично, HIGH confidence). Потребителят пусна резултата и веднага
се видяха 2 реални проблема в данните, не само в кода:

1. **Community score = 30 за почти всички ниши** — подозрително
   еднакво. Причина: Reddit вероятно блокира/rate-limit-ва GitHub
   Actions IP адреси (същия клас проблем като известния Google Trends
   429 в `track_trends.py`), затова почти винаги пада до archive/
   неутрален placeholder.
2. **Demand опира таван (100) прекалено лесно** — 3 от 4-те топ ниши
   показваха Demand=100 едновременно, губи се разграничение между
   "голяма" и "огромна" ниша. Причина: фиксиран праг
   (`market/10_000_000*100`), не относителна нормализация — точно
   грешката, която оригиналният дизайн (началото на разговора същия
   ден) изрично предупреждаваше да се избягва.

**Фикс:**
- `fetch_discogs_community()` — нов основен Community източник
  (`api.discogs.com/database/search`, release count като прокси за
  фен/колекционерска дълбочина). Reddit остава като резерва, не
  основен вече.
- `collect_raw_signals()` + `finalize_scores()` — `score_niche()`
  разбит на 2 прохода. Първо се теглят суровите Demand (market
  estimate) и Community (release/subscriber count) числа за ВСИЧКИ
  ниши, после `_portfolio_normalize()` прави min-max между тях
  (0-100 спрямо най-малката/най-голямата ниша В ТОЗИ скен, не спрямо
  фиксиран праг). Ако всички стойности са равни (или само 1 ниша) →
  неутрално 50 за всички, без деление на нула. `None` (липсващи данни)
  не се третира като 0 — не дърпа изкуствено скалата надолу.
  Momentum/Opportunity не се пипат — вече бяха 0-100 от собствена
  формула (растеж %, HHI), не фиксиран праг.
- `js/niche-toolkit.js` — Community клетката в таблицата вече показва
  `title` tooltip с реалния източник (Discogs/Reddit/архив).

**Проверка:** `py_compile` чисто, `node --check` чисто, `npm test` →
73/73 непроменено. Локален unit тест (без мрежа) на
`_portfolio_normalize()`: `[10,50,100]→[0,44.4,100]`,
`[5,5,5]→[50,50,50]` (без деление на нула), `[None,50,100]→[50,0,100]`
(липсващи данни не чупят скалата). Живите Discogs заявки НЕ са
тествани на живо — очаква се следващото пускане (понеделник) за
потвърждение.
...
endpoint-ът е споделен между ВСИЧКИ анонимни потребители в интернет,
без ключ няма собствена квота. Добавен автоматичен retry в
`pollinationsImageUrlAsync()` (`js/providers/pollinations-image.js`):
до 3 опита с нарастващо изчакване (0s → 2s → 5s) и нов случаен `seed`
на всеки опит (заобикаля евентуален CDN/кеш rate-limit на конкретен
URL). Ако и след 3-те опита пак гърми 429 — ясно съобщение към
потребителя да изчака ~30 сек и да натисне бутона отново, вместо тих/
неясен провал. **Не е тествано на живо** (средата за писане на кода
няма мрежов достъп до image.pollinations.ai) — очаква се повторно
потвърждение от потребителя.

**Addendum 2 (2026-08-12) — промптът на потребителя се игнорира/конкурира.**
Потребителят подаде промпт "3d logo cd-b records" и получи напълно
несвързани резултати при всяко генериране. Root cause, идентифициран
в `js/step3.js`: и `_generateCoverImagePollinations()`, и
`_generateCoverImageGemini()` добавяха ФИКСИРАН, hardcoded контекст
пред промпта на потребителя — `"Square album cover art, professional
streaming cover, 1:1 composition: "` / `"Square album cover art,
3000x3000px composition: "`. Когато потребителят иска нещо различно
от "обложка на албум" (в случая — лого), този добавен контекст
директно конкурира с исканото съдържание и моделът произвежда
несвързан резултат. **Fix:** премахнат hardcoded семантичен wrapper
и в двата метода — промптът на потребителя вече се подава практически
буквално, само с технически quality-суфикс накрая (`", high quality,
high resolution, sharp focus"` / `", square 1:1 composition, high
resolution"`), който не пренаписва съдържанието.
**Честно ограничение, което остава и НЕ е "бъг за оправяне":** дори
след fix-а, безплатни дифузионни модели (Flux през Pollinations) имат
ограничена, но реална граница за точно изписване на конкретен текст в
изображението (напр. точните букви "CD-B Records" четливо в 3D лого)
— това е известно, индустриално ограничение на текущите отворени
text-to-image модели, не проблем в промпт wrapper-а. На потребителя
беше обяснено честно и предложена алтернатива (code-генерирано
типографско лого с гарантиран точен текст, Canvas/CSS, без AI
randomness) — потребителят засега избра да продължи с чист AI подход
без hardcoded wrapper; не е поискано code-лого решението.

**Addendum 3 (2026-08-12) — потребителят поиска и двата варианта едновременно.**
Последвал разговор: потребителят поиска code-логото ДА се добави като
трета опция, не като замяна — за да избира сам кое пасва за случая.
Добавен `js/providers/code-logo.js` (`renderCodeLogo()`, чист Canvas
2D — extrusion + градиент + bevel + сянка, без мрежова заявка изобщо,
без ключ) и `Step3.generateCoverImageCodeLogo()` (нов бутон "🔤
Генерирай код-лого" + две нови input полета: `#codeLogoText`
по подразбиране "CD-B Records", `#codeLogoSubtitle` по избор). Версия
вдигната на v1.27.1. Верификация: `node --check` чисто на
`code-logo.js` и `step3.js`, `<div>` баланс в `index.html` проверен
(345/345). **Не е тествано на живо в реален браузър** (само статичен
синтаксис/баланс проверен от средата за писане на кода) — очаква се
потвърждение от потребителя как изглежда реалният canvas резултат.

**Addendum (2026-08-13) — AI-fingerprint gate за текстовете на песните.**
Нов модул `js/lyrics-humanizer.js` (`LyricsHumanizer`) — правило-базирана
(без допълнителен AI call) детекция дали генериран текст "звучи AI":
cliché фрази (english+БГ списък, разширяем), прекалено еднаква дължина на
редовете (ниска вариация), повторение на идея между [Verse] секции
(word-overlap евристика). `evaluate()` връща `{score, flags, pass}` спрямо
`THRESHOLD = 40`.
Интегриран в `Step1.generateLyrics()` (`js/step1.js`): след генериране,
ако `evaluate().pass === false`, автоматично се прави до `MAX_RETRIES = 2`
повторни опита с усилен промпт, който изрично изброява засечените
проблеми за пренаписване. Ако и след retry-ите пак fail, показва се
badge (🟡/🔴) + списък флагове + ръчен бутон "🔁 Регенерирай" до текста;
при pass — тих 🟢 badge.
UI: нов `<div id="humanizerOut">` под `#lyricsOut` в `index.html`.
Добавен `js/lyrics-humanizer.js` в `<script>` списъка на `index.html` и
в `SHELL_FILES` на `sw.js` (CACHE_VERSION вдигнат v47 → v48).
Няма нови зависимости, няма нов storage ключ, нищо в други модули не е
пипнато.
Тестове: нов `test/lyrics-humanizer.test.mjs` (8 теста: cliché match,
структурна симетрия, повторение между куплети, pass/fail spрямо
threshold) + `test/load-lyrics-humanizer.mjs` (vm loader helper, същия
патерн като `load-niche-scoring.mjs`). Пълен suite: **81/81** (73 стари
+ 8 нови), `node --check` чисто на трите пипнати/нови JS файла, `<div>`
баланс в `index.html` проверен (358/358).
**НЕ е тествано на живо в реален браузър/Suno** — очаква се потвърждение
от потребителя как изглеждат badge/flags резултатите с реални генерации.

**Addendum 2 (2026-08-13) — v2: AI-fingerprint gate заменен с AI-съдия (Gemini).**
Потребителят тества v1 gate-а с реален пример ("Счупих телефона в бара с
огън в душата...") и коректно флагна, че звучи като поема, не песен —
rule-based `detect()` върна **score 0, 0 флага** (пълен пропуск).
Root cause: regex/keyword списък срещу клишета е лесно заобиколим от
перифраз ("огън в душата" не беше в списъка) и структурно не може да
прецени "поема vs песен" (сложни подчинени изречения, затворена
наративна дъга) — това изисква реална преценка, не точно съвпадение.
Обсъдена и отхвърлена алтернатива: ползване на textove.com като "база"
за реални текстове — отхвърлено заради copyright риск (съхранение на
чуждо защитено съдържание), не само заради техническа неефективност.
**Fix (v2):** нов метод `LyricsHumanizer.judge(lyrics)` (async) —
основният gate вече е AI-съдия през `callGemini()`, с промпт, който
изрично разграничава признаци на "истинска песен" срещу "AI поема"
(кратки singable редове vs дълги сложни изречения, повтарящ се
емоционален момент vs затворена наративна дъга, конкретни детайли vs
абстрактни двойки образи). `detect()`/`evaluate()` (v1, rule-based)
остават като бърз безплатен допълнителен сигнал, подаван В промпта на
съдията — вече не вземат самостоятелно финалното pass/fail решение.
Ако Gemini ключ липсва/API гръмне — тих fallback към rule-based резултат
(`source: "rule-fallback"`), не блокира `generateLyrics()`; UI показва
бележка "(Gemini недостъпен — бърза проверка, по-малко надеждна)" в
такъв случай.
`Step1.generateLyrics()` обновен: `evaluate()` → `await judge()` (вече
async), retry промптът вече споменава изрично и "звучи като поема".
Тестове: `test/load-lyrics-humanizer.mjs` обновен да приема mock
`callGemini`/`extractJson` през sandbox (без реална мрежова заявка);
4 нови теста за `judge()` (pass/fail от mock Gemini отговор, fallback
при грешка, и регресионен тест точно с реалния пример на потребителя,
потвърждаващ, че AI съдията го хваща там, където v1 detect() пропусна).
Пълен suite: **85/85** (81 стари + 4 нови). `node --check` чисто,
`<div>` баланс в `index.html` непроменен (358/358).
**НЕ е тествано на живо с реален Gemini ключ в браузър** — mock
тестовете потвърждават логиката (parse на JSON отговор, pass/fail
изчисление, fallback при грешка), но не реалното качество на преценките
на живия Gemini модел върху разнообразни текстове. Очаква се
потвърждение от потребителя след реален тест.
