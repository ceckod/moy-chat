...
**Договорен (но ОЩЕ НЕ построен) discovery слой — 4 независими
източника, за да не разчитаме само на YouTube:**
1. YouTube Trending Chart (`chart=mostPopular&videoCategoryId=10`) —
   топ музикални видеа в тренда точно сега, извличане на повтарящи се
   термини от заглавия/тагове.
2. Wikipedia Pageviews — "най-растящи" статии от категория "Music
   genres" (внезапен скок в прегледите = ново набиращо популярност).
3. MusicBrainz — жанрови тагове, добавяни най-често наскоро към нови
   артисти/издания.
4. Reddit — нови/бързо растящи music subreddit-и
   (`reddit.com/subreddits/new.json` + филтър).

Логика: термин, засечен от ≥2 от четирите източника независимо, става
"кандидат-ниша" и автоматично влиза в следващия седмичен PNS скен.
Целта е нула ръчна намеса от потребителя на който и да е етап —
нито seed списък, нито одобрение на кандидати (изрично потвърдено:
"искам да ми дава най-актуалното", директно в класацията, не одобрение
стъпка по стъпка).

**Следваща сесия, по приоритет:**
1. Discovery слоят по-горе (4 източника) — все още само дизайн, 0 код.
2. Discogs, ListenBrainz, YouTube RSS — допълнителни Demand/Community
   сигнали за самия PNS Scanner, съзнателно оставени за после.
3. `config.json` → `niche_scan_niches` с per-ниша feasibility/
   monetization (ако/когато потребителят иска персонализация отвъд
   неутралните placeholder-и).
4. Първо реално пускане на `niche-scores.yml` от потребителя, за да
   потвърдим живите мрежови резултати (виж "Проверка" по-горе).
